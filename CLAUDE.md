...
- Before the first code edit of a session, OFFER alignment (`/grill-me` or `/grill-with-docs`) instead of coding straight from the request. If the work is trivial or already aligned, say so and retry the edit: the call is the user's, not the gate's. This breaks the "fix→implement" autopilot that text rules alone do not stop, and what the `alignment-gate` hook watches, what it lets through and how often it fires is in `docs/ai-workflow/AI_DEVELOPMENT_WORKFLOW.md` § 1, not here.
...
- Las tres skills bootstrap (`southpoint`, `personal`, `ai-project`) se mantienen **espejadas en estructura**: si cambiás la mecánica (Step 0–5 del SKILL.md), aplicá el mismo cambio en las tres. Solo difieren en: contenido DOMO (solo Southpoint), issue tracker (Zoho en las tuyas, genérico en ai-project), catálogo MCP, identidad git (ai-project no la toca), SKILL.md y los archivos sólo-Southpoint que declara `tests/lib/solo-southpoint.ps1` (hoy el recolector de hub-sync): un archivo que existe en una sola skill va en esa lista, no copiado a las tres. `tests/mirror.tests.ps1` verifica la byte-identidad de todo lo demás y que los sólo-Southpoint no aparezcan en las otras dos; `tests/shareable-leaks.tests.ps1` garantiza que lo publicable no filtre datos personales.
