...
After approval: insert operational-rule blocks into `## Hard rules` as new bullets (verbatim); append domain blocks under `## Project-specific domain` in `docs/agents/domain.md` (verbatim); seed `CONTEXT.md` with the description. **Then apply the rest of the map** — every `overwritten` row the user approved as "restore" or "merge" is work nobody else does: copy the file back from its `backup` path, or merge the two by hand. A row approved and not applied is the same silent revert the map exists to prevent. Leave the original untouched as the permanent backup.
