...
This delivers: `CLAUDE.md`, `.gitignore`, `.gitattributes` (keeps `*.sh` in LF on every checkout, so bash on Linux or WSL can run them), `skills-lock.json`, `.bootstrap-manifest.json` (scaffold version manifest, used by `upgrade-bootstrap`), `.agents/skills/` (21 skills — 17 synced via `skills-lock.json` + `review-loop`, `slice-review`, `verify-downstream-arrival` and `debug-source-first`, bundled here), `.claude/commands/` (21 commands), `.claude/agents/` (the read-only reviewer agents `/slice-review` fans out to, with the file-mutating tools denied by declaration), `.claude/settings.json` + `.claude/hooks/review-loop-trigger.ps1` (auto-dispara `review-loop` al abrir/actualizar un PR) + `.claude/hooks/alignment-gate.ps1` (frena el primer edit de código por sesión y ofrece alinear antes de codear), `docs/ai-workflow/` (11 docs), `docs/agents/` (3 docs).

## Step 3 — Project-specific files

Create these **only where they do not already exist** (they are per-project, so they are not in the scaffold — which also means `copy-scaffold.ps1` never backed them up and they are in no `overwritten` list: overwrite one and it is gone for good, unlike every other file this skill touches). In adoption mode an existing `README.md` or `CONTEXT.md` is the project's own; leave it exactly as it is.
