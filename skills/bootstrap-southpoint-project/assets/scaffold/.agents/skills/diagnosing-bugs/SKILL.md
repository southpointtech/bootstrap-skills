...
10. **Human in the loop.** Last resort. If a human must click, keep the loop structured anyway: write the steps as a numbered checklist, each with one action and the one observation to report back (answers like `ERRORED=y`), ask the user to run it and paste the answers back, redacted, and re-run the same checklist on every iteration. Upstream's bash template for this (`scripts/hitl-loop.template.sh`) is not shipped here: the agent's shell cannot hand its prompts to the human. With stdin closed the template exits 1 at its first prompt, and with stdin left open it blocks on input that nobody can type.
...
- [ ] **Agent-runnable**: you can run it unattended; a human in the loop only through the checklist of item 10.
