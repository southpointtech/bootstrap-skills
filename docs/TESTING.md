...
- **Gramática de comillas por herramienta** (issue 24) — `Hide-Literals` parseaba con comillas de bash, donde `\` escapa, y el matcher ensanchado del issue 21 le empezó a entregar comandos de PowerShell, donde `\` es un carácter común y una ruta entrecomillada **termina** en `\`. El walker se pasaba del cierre, enmascaraba hasta el fin de línea y el disparador desaparecía: falso negativo mudo. Ahora la gramática se elige por el `tool_name` del evento. Los nueve comandos de la tabla del issue se escribieron como asserts **contra el hook sin tocar** antes del fix y fallaron exactamente los **cuatro** marcados `NO` (el AC original decía seis; ese número no salía de una corrida). Cubierto: los 4 del bug, 4 controles positivos (commit pelado, `git -C` sin `\` final, comillas simples, sin comillas), los falsos positivos que la gramática nueva tiene que seguir conteniendo (mensaje que menciona `git push`, comilla doble duplicada `""`, apóstrofe duplicado `''`, y `\` delante de la comilla de apertura), el costo aceptado del backtick (que dispara por el **recomputo crudo**, no por la gramática), **dos de regresión** que fijan que `Bash` conserva su gramática (`\"` sigue siendo escape, `'\''` sigue sin exponer el mensaje), y **dos `gh pr create` desde PowerShell** que son los únicos casos que llegan al otro lector del enmascarado — la lectura de `--base` del paso 4 — y los únicos que fijan las dos ramas del backtick (a nivel tope y dentro de comillas dobles). Un evento **sin** `tool_name` cae a bash, que es la conducta previa; lo ejercita la enorme mayoría de los casos del archivo, que no mandan la clave (las excepciones son los tres del despacho, que desde este slice sí la mandan).

  Distinción que conviene no perder, porque dos de esos casos no fijan lo que parece: los de `""` y `''` **no** mueren si se saca la rama de la comilla duplicada (las comillas se re-alinean y `git push` queda enmascarado igual); son contención de falsos positivos en la forma realista de PowerShell, con el mismo poder discriminante que el caso del mensaje que menciona `git push`. Mutantes medidos, uno por uno: **ignorar `tool_name` mata cinco** asserts (los 4 del bug y el del `\` delante de la comilla); **volver `$esc` a `\` en las dos gramáticas mata uno**, el del `\`; **anular sólo la mitad PowerShell de `$esc`** (que el backtick sea el escape de afuera) mata el `gh pr create` con backtick a nivel tope; **sacar la rama del backtick dentro de comillas dobles** mata el otro `gh pr create`. Los dos últimos sobrevivían con la suite entera en verde hasta que se agregaron esos dos casos.
...
- Una rama de la gramática de PowerShell de `Hide-Literals`: la **comilla duplicada** (`""` / `''`). Sobrevive a su mutante y se mantiene igual, porque modela la gramática real — dejar el modelo a medias es exactamente lo que causó el issue 24. No es cobertura que falte por descuido: `tools/fuzz-hide-literals.ps1` la mide por fuerza bruta y no encuentra ningún comando bien formado en que cambie una bandera del paso 2 ni la lectura de `--base` del paso 4. El barrido ciego llega a 4 piezas, y el propio script advierte en su salida que "sin contraejemplo" significa que **esta** búsqueda no lo encontró, no que no exista: su alfabeto no tiene here-strings, ni el token `--%`, ni rutas UNC. Correlo antes de tocar el walker; imprime los conteos y el primer contraejemplo de cada rama.
  ...
  Esa búsqueda dejó una lección que vale más que el resultado. En su primera versión medía sólo las **tres banderas** del paso 2 y concluyó que las otras dos ramas de la gramática —el escape de **afuera** del literal y el **backtick dentro de comillas dobles**— tampoco eran observables. Era falso: las dos son observables por el **otro** lector del enmascarado, la lectura de `--base`, que la búsqueda no modelaba. Las dos sobrevivían a su mutante con la suite entera en verde, y las cazó el foco de mutación del review-loop, no la búsqueda. Hoy las dos tienen su caso (los dos `gh pr create` desde PowerShell) y la búsqueda mide los dos canales. Antes de declarar una rama "no observable", chequeá contra qué se midió.
