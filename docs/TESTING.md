...
- **Gramática de comillas por herramienta** (issue 24) — `Hide-Literals` parseaba con comillas de bash, donde `\` escapa, y el matcher ensanchado del issue 21 le empezó a entregar comandos de PowerShell, donde `\` es un carácter común y una ruta entrecomillada **termina** en `\`. El walker se pasaba del cierre, enmascaraba hasta el fin de línea y el disparador desaparecía: falso negativo mudo. Ahora la gramática se elige por el `tool_name` del evento. Los nueve comandos de la tabla del issue se escribieron como asserts **contra el hook sin tocar** antes del fix y fallaron exactamente los **cuatro** marcados `NO` (el AC original decía seis; ese número no salía de una corrida). Cubierto: los 4 del bug, 4 controles positivos (commit pelado, `git -C` sin `\` final, comillas simples, sin comillas), los falsos positivos que la gramática nueva tiene que seguir conteniendo (mensaje que menciona `git push`, comilla doble duplicada `""`, apóstrofe duplicado `''`, y `\` delante de la comilla de apertura), el costo aceptado del backtick (cae al recomputo crudo y dispara), y **dos de regresión** que fijan que `Bash` conserva su gramática (`\"` sigue siendo escape, `'\''` sigue sin exponer el mensaje). Un evento **sin** `tool_name` cae a bash, que es la conducta previa, y eso lo ejercitan todos los casos anteriores del archivo, que no mandan la clave. Mutantes medidos: ignorar `tool_name` mata los 4 asserts del bug; volver `$esc` a `\` en PowerShell mata el del `\` delante de la comilla.

**Lo que este archivo de tests NO cubre** (verificado por mutación, no inferido):

- Dos ramas de la gramática de PowerShell de `Hide-Literals`: la **comilla duplicada** (`""` / `''`) y el **backtick dentro de comillas dobles**. Las dos sobreviven a su mutante. No es cobertura que falte por descuido: se midió que no son observables en lo que el hook **decide**. Por fuerza bruta sobre un alfabeto de piezas (comillas, backtick, `\`, `git push`, `git commit`, texto, espacio) hasta 5 piezas — 37.448 comandos, 16.398 de ellos bien formados —, ninguna de las dos cambia ninguna de las tres banderas del paso 2, recomputo crudo incluido. Sí cambian el enmascarado, y el otro lector del enmascarado es la lectura de `--base` del paso 4, para la que no hay fixture con estas formas. Se mantienen porque modelan la gramática real: dejar el modelo a medias es exactamente lo que causó el issue 24. La tercera rama, el escape de **afuera** del literal, sí cambia el disparo (`\""git push`, hallado por la misma búsqueda) y por eso tiene su propio caso.
