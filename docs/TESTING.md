...
- **Score the FIX** — el scorer responde tres preguntas y las declara: el hecho se verifica **corriendo un comando** (`running a command against the real code`), el **reemplazo** propuesto se chequea igual (`check the *replacement* the same way`) y arreglar 1 de N ocurrencias idénticas implica haber auditado las otras `N-1`. Fallar (2) o (3) descarta el hallazgo **bajo 60** por certero que sea (1) — el assert se ancla al **vínculo** entre el fallo y el corte, no a las dos frases por separado. Más el contra-argumento de alcance (defecto DEL delta vs. condición preexistente) y un guard de que el bloque entra como canónico, sin los marcadores `PATCH:prose-churn`. Ver ADR-0010.
