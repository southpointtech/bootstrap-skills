...
- **Score the FIX** — el scorer responde tres preguntas **obligatorias** y las declara, y el **número 0-100 contesta (1) sola** (`The 0-100 score answers (1) alone`): si (2)/(3) se le restaran, el hallazgo moriría en el corte de 60 antes de clasificarse. El hecho se verifica con un comando **read-only** —regla general, sin afirmar nada sobre la suite del repo destino—, el **reemplazo** se chequea igual y aplica al arreglo en cualquier forma, y arreglar 1 de N ocurrencias exige que las otras `N-1` **hayan sido auditadas**. Una pregunta que no aplica no es un fallo. La consecuencia se ancla por oración completa: un **Low** que falla se descarta; un **Medium o High** sobrevive con el arreglo marcado `REJECTED`, **conserva su severidad**, y el Step 6 lo declara uno por uno (cuenta **contra** clean). El orden se verifica **por posición** (`IndexOf` de la clasificación vs. el párrafo del REJECTED), no por presencia. Del lado del caller, tres asserts sobre `/review-loop`: qué significa REJECTED, y que un rango vacío con un Medium/High real sin arreglar es cierre por techo o bloqueado, nunca limpio. Y como los asserts semánticos son ciegos a los mutantes por **añadido** (medido: una cláusula de excepción al final deja todo verde), el bloque queda congelado por un **golden por hash** — `tests/fixtures/step5-score-the-fix.golden.sha256`, resellable sólo con `tools/reseal-step5.ps1`, cuyo guard exige que las 8 copias coincidan antes de sellar. Ver ADR-0010.
...
- **Despacho por herramienta** (issue 21) — el sujeto es el `matcher` del `PostToolUse` de `settings.json`, que es lo único que decide si el hook **corre**. Un cierre declarado desde la herramienta **PowerShell** despacha; desde **Bash** sigue despachando; `Edit` no despacha (mata un matcher perezoso `.*`). El hook también lee `tool_name`, pero para otra cosa —elegir la gramática de comillas, abajo—, y un matcher que no empareja gana antes: el hook no llega a correr. Más dos invariantes de config: el matcher es idéntico en las **4 raíces** y es exactamente `Bash|PowerShell` (con `-ceq`, porque `-Unique` atrapa que UNA raíz difiera, no que las cuatro tengan el mismo error de tipeo), y el del **alignment-gate** sigue en `Edit|Write|MultiEdit` en las cuatro, para que ensanchar uno no arrastre al otro.

- **Gramática de comillas por herramienta** (issue 24) — `Hide-Literals` parseaba con comillas de bash, donde `\` escapa, y el matcher ensanchado del issue 21 le empezó a entregar comandos de PowerShell, donde `\` es un carácter común y una ruta entrecomillada **termina** en `\`. El walker se pasaba del cierre, enmascaraba hasta el fin de línea y el disparador desaparecía: falso negativo mudo. Ahora la gramática se elige por el `tool_name` del evento. Los nueve comandos de la tabla del issue se escribieron como asserts **contra el hook sin tocar** antes del fix y fallaron exactamente los **cuatro** marcados `NO` (el AC original decía seis; ese número no salía de una corrida). Cubierto: los 4 del bug, 4 controles positivos (commit pelado, `git -C` sin `\` final, comillas simples, sin comillas), los falsos positivos que la gramática nueva tiene que seguir conteniendo (mensaje que menciona `git push`, comilla doble duplicada `""`, apóstrofe duplicado `''`, y `\` delante de la comilla de apertura), el costo aceptado del backtick (que dispara por el **recomputo crudo**, no por la gramática), **dos de regresión** que fijan que `Bash` conserva su gramática (`\"` sigue siendo escape, `'\''` sigue sin exponer el mensaje), y **dos `gh pr create` desde PowerShell**, los únicos que llegan al otro lector del enmascarado —la lectura de `--base` del paso 4— **en gramática de PowerShell**, y los únicos que fijan las dos ramas del backtick (a nivel tope y dentro de comillas dobles); los otros fixtures de `--base` que ya existían corren en gramática de bash. Un evento **sin** `tool_name` cae a bash, que es la conducta previa, y eso lo ejercitan todos los casos del archivo anteriores al bloque del despacho, que no mandan la clave.

  Distinción que conviene no perder, porque dos de esos casos no fijan lo que parece: los de `""` y `''` **no** mueren si se saca la rama de la comilla duplicada (las comillas se re-alinean y `git push` queda enmascarado igual); son contención de falsos positivos en la forma realista de PowerShell, con el mismo poder discriminante que el caso del mensaje que menciona `git push`. Mutantes medidos, uno por uno: **ignorar `tool_name` mata cinco** asserts (los 4 del bug y el del `\` delante de la comilla); **volver `$esc` a `\` en las dos gramáticas mata dos**, el del `\` y el `gh pr create` con backtick a nivel tope; **anular sólo la mitad PowerShell de `$esc`** mata ese mismo `gh pr create`; **sacar la rama del backtick dentro de comillas dobles** mata el otro. Los dos `gh pr create` sobrevivían con la suite entera en verde hasta que se agregaron esos dos casos.

  Los conteos de este párrafo se desactualizan cada vez que el bloque crece, y ya pasó: el turno que agregó los dos `gh pr create` dejó "mata uno" donde ya eran dos, en la misma pasada en que corregía el mismo tipo de error tres líneas más arriba. Si agregás un caso, **volvé a medir los cuatro mutantes** antes de tocar este texto.

**Lo que este archivo de tests NO cubre** (verificado por mutación, no inferido):

- Una rama de la gramática de PowerShell de `Hide-Literals`: la **comilla duplicada** (`""` / `''`). Sobrevive a su mutante y se mantiene igual, porque modela la gramática real — dejar el modelo a medias es exactamente lo que causó el issue 24. No es cobertura que falte por descuido: `tools/fuzz-hide-literals.ps1` la mide por fuerza bruta y no encuentra ningún comando bien formado en que cambie una bandera del paso 2 ni la lectura de `--base` del paso 4. El alcance del barrido no se repite acá a propósito — el script lo imprime en su salida junto con los conteos, y este párrafo ya tuvo una vez el número viejo después de que el barrido se ensanchara. El propio script advierte que "sin contraejemplo" significa que **esa** búsqueda no lo encontró, no que no exista: su alfabeto no tiene here-strings, ni el token `--%`, ni rutas UNC. Correlo antes de tocar el walker; imprime los conteos y el primer contraejemplo de cada rama.

  Esa búsqueda dejó una lección que vale más que el resultado. En su primera versión medía sólo las **tres banderas** del paso 2, y sobre esa base se declaró no observable el **backtick dentro de comillas dobles**. Era una conclusión de más: esa rama es observable por el **otro** lector del enmascarado, la lectura de `--base`, que la búsqueda no modelaba. Lo mismo valía para la mitad PowerShell del escape de **afuera** del literal, que quedó sin caso aunque su otra mitad sí lo tenía. Las dos sobrevivían a su mutante con la suite entera en verde, y las cazó el foco de mutación del review-loop, no la búsqueda. Hoy las dos tienen su caso (los dos `gh pr create` desde PowerShell) y la búsqueda mide los dos canales. Antes de declarar una rama "no observable", chequeá contra qué se midió — y contra qué **no**.
