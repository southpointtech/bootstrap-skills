...
- **Score the FIX** — el scorer responde tres preguntas y las declara: el hecho se verifica corriendo un comando **read-only** (`Verify it by running a **read-only** command against the real code`), el **reemplazo** propuesto se chequea igual —con su contraste— y aplica al arreglo en cualquier forma, y arreglar 1 de N ocurrencias idénticas exige que las otras `N-1` **hayan sido auditadas**. Una pregunta que no aplica no es un fallo. La consecuencia se ancla por oración completa, no por ventana de distancia: un **Low** que falla (2)/(3) cae bajo 60 por certero que sea (1); un **Medium o High** sobrevive con el arreglo marcado `REJECTED` y sigue bloqueando el cierre, y el Step 6 lo declara. Más el contra-argumento de alcance con su consecuencia, el guard de que el dispatch del scorer lleva la prohibición de escritura del Step 3, y cuatro **guardas de negación** (`no need to`, `need not check`, `not by running`, `can be left as they are`): en el turno 1 del review-loop 4 de 8 mutantes sobrevivían porque negar la regla conservaba el sustantivo anclado, y con estos anclajes los 6 mueren. Ver ADR-0010.
